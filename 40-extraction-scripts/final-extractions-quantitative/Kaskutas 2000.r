...
res$sample$recruitmentSetting <- "prenatal clinics"; ### For example "antenatal clinic",or NA
...
res$univariate[[length(res$univariate) + 1]] <- list(
  variable = "Is it ever to late to quit drinking",       ### Name of variable this result pertains to
...
res$variable[[length(res$variable) + 1]] <- list(
  variable = "Is it ever to late to quit drinking",             ### Replace 'example' with variable name
  moment = 1,                       ### moment this variable was measured/manipulated
  type = "question",                ### "question", "aggregate", or "manipulation"
  datatype = "logical",             ### "numeric", "logical", "nominal", "ordinal", or "text"
  values = NA,                      ### Possible values; NA or a vector, e.g. c(1, 2, 3)
  labels = NA,                      ### Labels for the values; NA or e.g. c("no", "maybe", "sometimes")
  description = "Is it ever too late to quit drinking or cut down, even if it's later in pregnancy (p.1245 table 4)",					###	Provide the full description of the variable (e.g., questionnaire question)
