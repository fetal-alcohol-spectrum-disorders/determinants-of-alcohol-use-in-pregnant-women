...
res$sample$recruitmentSetting <- "primary health care clinic"; ### For example "antenatal clinic",or NA
...
res$sample$geography <- "United States"				### all Mexican origin, interview in Spanish lang.
...
res$variable[[length(res$variable) + 1]] <- list(
  variable = "alcohol can hurt the unborn child",             ### Replace 'example' with variable name
  ...
  labels = c("disagree", "unsure", "agree"),                      ### Labels for the values; NA or e.g. c(no", "maybe", "sometimes")
  description = "Drinking alcohol can hurt an unborn child (p.5 table 1)",	###	Provide the full description of the variable (e.g., questionnaire question)
...
res$variable[[length(res$variable) + 1]] <- list(
  variable = "alcohol can help you when pregnant",             ### Replace 'example' with variable name
  moment = 1,                       ### moment this variable was measured/manipulated
  type = "question",                ### "question", "aggregate", or "manipulation"
 datatype = "nominal",          		### "numeric", "logical", "nominal", "ordinal", or "text"
  values = c(1, 2, 3),                      ### Possible values; NA or a vector, e.g. c(1, 2, 3)
  labels = c("disagree", "unsure", "agree"),                      ### Labels for the values; NA or e.g. c(no", "maybe", "sometimes")
  description = "Drinking alcohol can help you when you are pregnant (p.5 table 1)",	###	Provide the full description of the variable (e.g., questionnaire question)
...
####################################### ONLY FOR DETERMINANTS OF MATERNAL DRINKING BEHAVIOR VARIABLES
  ...
  period = NA,               		### "before", "1st", "2nd", "3rd", "during", "after", "other"
  determinant_original = "knowledge",		### If reported, how do the authors categorized the determinant, e.g., "knowledge", "attitude", "risk perception"
  determinant_recode = "attitude positive",			### Determinant recoded after expert meeting 
