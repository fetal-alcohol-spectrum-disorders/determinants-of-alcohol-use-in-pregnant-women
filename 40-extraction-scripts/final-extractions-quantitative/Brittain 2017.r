...
These results only pertain to women who were classified as hazardous drinkers (among women reporting any alcohol use in the 12 months prior to pregnancy, hazardous (AUDIT-C score ≥3) versus non-hazardous (AUDIT-C score <3) alcohol use. (p. 71, section 2.4, bottom right)
...
res$sample$recruitmentSetting <- "prevention of mother-to-child transmission services"; ### For example "antenatal clinic",or NA

### Sample Method
res$sample$native <- NA; ### TRUE for native/aboriginal populations (e.g. inuit, aboriginals, etc)

### Description of sample, in text
res$sample$description <- "HIV infected pregnant women"
...
res$sample$size <- 152;			### Hazardous use prior
...
res$association[[length(res$association) + 1]] <- list(
  ...
  OR = NA,                          ### Odds ratio
  p = .169,                           ### P-value (if nothing else is available)
...
res$variable[[length(res$variable) + 1]] <- list(
  variable = "perceivedSupportAvailability",             ### Replace 'example' with variable name
  moment = 1,                       ### moment this variable was measured/manipulated
  type = "question",                ### "question", "aggregate", or "manipulation"
  datatype = "numeric",             ### "numeric", "logical", "nominal", "ordinal", or "text"
  values = NA,                      ### Possible values; NA or a vector, e.g. c(1, 2, 3)
  labels = NA,                      ### Labels for the values; NA or e.g. c("no", "maybe", "sometimes")
  description = "table 3 page 74",					###	Provide the full description of the variable (e.g., questionnaire question)
...
####################################### ONLY FOR DETERMINANTS OF MATERNAL DRINKING BEHAVIOR VARIABLES
  ...
  determinant_recode = "general perceived social support",			### Determinant recoded after expert meeting 
...
res$variable[[length(res$variable) + 1]] <- list(
  variable = "drinking",             ### Replace 'example' with variable name
  moment = 1,                       ### moment this variable was measured/manipulated
  type = "question",                ### "question", "aggregate", or "manipulation"
  datatype = "numeric",             ### "numeric", "logical", "nominal", "ordinal", or "text"
  values = NA,                      ### Possible values; NA or a vector, e.g. c(1, 2, 3)
  labels = NA,                      ### Labels for the values; NA or e.g. c("no", "maybe", "sometimes")
  description = "[...] continued hazardous use (AUDIT-C score >= 3) versus reduced consumption (AUDIT-C score <3) between pregnancy recognition and late third trimester. (p. 71, bottom of the page, right column, section 2.4",					###	Provide the full description of the variable (e.g., questionnaire question)
