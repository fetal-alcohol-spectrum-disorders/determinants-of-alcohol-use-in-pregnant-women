...
res$sample$description <- "women with young children"
...
res$variable[[length(res$variable) + 1]] <- list(
  ...
  description = "...We get differing things out all the time as to probably none’s okay and then suddenly, well, you’re probably okay with one glass of wine or something, so the information is confusing (p.526)",					###	Provide the full description of the variable (e.g., questionnaire question)
...
res$variable[[length(res$variable) + 1]] <- list(
  ...
  values = NA,                      ### Possible values; NA or a vector, e.g. c(1, 2, 3)
  labels = NA,                      ### Labels for the values; NA or e.g. c("no", "maybe", "sometimes")
  description = "Culturally we’re just so—drinking’s such an accepted form of behavior . . .  birthdays, Christmases, Easter Friday, Saturday football, Sunday-afternoon family gatherings, my god, is there anything that doesn’t have grog involved? (p.527)",					###	Provide the full description of the variable (e.g., questionnaire question)
...
####################################### ONLY FOR DETERMINANTS OF MATERNAL DRINKING BEHAVIOR VARIABLES
  ...
  period = NA,               		### "before", "1st", "2nd", "3rd", "during", "after", "other"
  determinant_original = NA,		### If reported, how do the authors categorized the determinant, e.g., "knowledge", "attitude", "risk perception"
  determinant_recode = "descriptive,subjective norm",			### Determinant recoded after expert meeting 
  determinant_target = "self,other",			###	"self", "other", or both "self,other"
...
res$variable[[length(res$variable) + 1]] <- list(
  ...
  values = NA,                      ### Possible values; NA or a vector, e.g. c(1, 2, 3)
  labels = NA,                      ### Labels for the values; NA or e.g. c("no", "maybe", "sometimes")
  description = "...We’re probably much more rigid about talking about giving up smoking Imust admit . . . we don’t – [we]wouldn’t say have the occasional cigarette . . . (p.527)",					###	Provide the full description of the variable (e.g., questionnaire question)
...
res$variable[[length(res$variable) + 1]] <- list(
  ...
  values = NA,                      ### Possible values; NA or a vector, e.g. c(1, 2, 3)
  labels = NA,                      ### Labels for the values; NA or e.g. c("no", "maybe", "sometimes")
  description = ". . . You very often hear, you know, ‘I drink but that’s okay’ cause I don’t smoke pot and don’t take [other drugs],’ like it’s, it seemed to be a lot lower down in the substance hierarchy (p.527)",					###	Provide the full description of the variable (e.g., questionnaire question)
