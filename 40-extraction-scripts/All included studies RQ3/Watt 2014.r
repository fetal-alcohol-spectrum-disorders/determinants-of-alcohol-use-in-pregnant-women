...
res$variable[[length(res$variable) + 1]] <- list(
  ...
  description = "Knowledge about the negative impact that alcohol consumption during pregnancy could have on a baby (p.6)",					###	Provide the full description of the variable (e.g., questionnaire question)
...
res$variable[[length(res$variable) + 1]] <- list(
  ...
  values = NA,                      ### Possible values; NA or a vector, e.g. c(1, 2, 3)
  labels = NA,                      ### Labels for the values; NA or e.g. c("no", "maybe", "sometimes")
  description = "Women used alcohol as a strategy to cope with stressors (e.g., 'I drink my problems away') (p.6) ",					###	Provide the full description of the variable (e.g., questionnaire question)
...
res$variable[[length(res$variable) + 1]] <- list(
  ...
  values = NA,                      ### Possible values; NA or a vector, e.g. c(1, 2, 3)
  labels = NA,                      ### Labels for the values; NA or e.g. c("no", "maybe", "sometimes")
  description = "Women drank as a way to retain social connection (e.g., 'just drinking to have fun')(p.6) ",					###	Provide the full description of the variable (e.g., questionnaire question)
...
res$variable[[length(res$variable) + 1]] <- list(
  ...
  values = NA,                      ### Possible values; NA or a vector, e.g. c(1, 2, 3)
  labels = NA,                      ### Labels for the values; NA or e.g. c("no", "maybe", "sometimes")
  description = "Social norms in women's peer groups supported drinking during pregnancy (e.g., 'in my area the women who are pregnant, they drink')(p.6)",					###	Provide the full description of the variable (e.g., questionnaire question)
...
res$variable[[length(res$variable) + 1]] <- list(
  ...
  values = NA,                      ### Possible values; NA or a vector, e.g. c(1, 2, 3)
  labels = NA,                      ### Labels for the values; NA or e.g. c("no", "maybe", "sometimes")
  description = "Women were driven physiologically by alcohol addiction (e.g., 'I drink too much, I never intended to become this way')(p.6)",					###	Provide the full description of the variable (e.g., questionnaire question)
...
####################################### ONLY FOR DETERMINANTS OF MATERNAL DRINKING BEHAVIOR VARIABLES
  ...
  period = NA,               		### "before", "1st", "2nd", "3rd", "during", "after", "other"
  determinant_original = NA,		### If reported, how do the authors categorized the determinant, e.g., "knowledge", "attitude", "risk perception"
  determinant_recode = "automaticity",			### Determinant recoded after expert meeting 
  determinant_target = "self",			###	"self", "other", or both "self,other"
  determinant_direction = "positive",		###	Indication if the direction of the determinant is "positive", "negative" or "unclear"
  determinant_consistent = "D",		###	Indication if the direction of the determinant is (in)consistent or unclear with the target/contrast behaviour  
