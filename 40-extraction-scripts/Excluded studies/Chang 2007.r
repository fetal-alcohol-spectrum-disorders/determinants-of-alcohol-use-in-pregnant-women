...
res$sample$description <- "randomized trial of brief interventions for prenatal alcohol use"
...
res$subsample[[length(res$subsample) + 1]] <- list( ### Create object for this subsample
  ...
  description = NA,
  size = 187                              ### Additional variables specified as univariate data for this subsample
)

res$subsample[[length(res$subsample) + 1]] <- list( ### Create object for this subsample
  name = "action",
  description = NA,
  size = 114                             ### Additional variables specified as univariate data for this subsample
)
...
res$association[[length(res$association) + 1]] <- list(
  ...
  OR = NA,                          ### Odds ratio
  p = .95,                           ### P-value (if nothing else is available)
  covariates = 0,                   ### Number of covariates (for multivariate analyses)
  comment = "table 1"                      ### Comment (e.g. calculation, page, etc)
);
...
res$association[[length(res$association) + 1]] <- list(
  ...
  var1values = NA,                  ### Values of var 1 in this analysis (e.g. post-hoc)
  var1moment = 1,                   ### Measurement moment of var 1
  variable2 = "temptation",     ### Name of second variable
  var2values = NA,                  ### Values of var 2 in this analysis (e.g. post-hoc)
  var2moment = 1,                   ### Measurement moment of var 2
  subsample = NA,                   ### Name of relevant subsample
  df1 = NA,                         ### degrees of freedom (numerator)
  ...
  etasq = NA,                       ### Eta squared value
  OR = NA,                          ### Odds ratio
  p = .04,                           ### P-value (if nothing else is available)
  covariates = 0,                   ### Number of covariates (for multivariate analyses)
...
res$univariate[[length(res$univariate) + 1]] <- list(
  variable = "temptation",       ### Name of variable this result pertains to
  subsample = "precontemplation",                   ### Name of relevant subsample, or NA
  ...
  comment = "scale ranges from 20 to 100"                      ### Comment or NA
);

res$univariate[[length(res$univariate) + 1]] <- list(
  variable = "confidence",       ### Name of variable this result pertains to
  subsample = "precontemplation",                   ### Name of relevant subsample, or NA
  value = NA,                       ### Value this result pertains to
  moment = 1,                       ### Measurement moment this datapoint pertains to
  minimum = NA,                     ### Minimum or NA
...
res$univariate[[length(res$univariate) + 1]] <- list(
  variable = "temptation",       ### Name of variable this result pertains to
  subsample = "action",                   ### Name of relevant subsample, or NA
  value = NA,                       ### Value this result pertains to
  moment = 1,                       ### Measurement moment this datapoint pertains to
  minimum = NA,                     ### Minimum or NA
  maximum = NA,                     ### Maximum or NA
  median = NA,                      ### Median or NA
  mean = 30.0,                        ### Mean or NA
  percentage = NA,                  ### Percentage that endorsed 1, or NA
  sd = NA,                          ### Standard deviation, or NA
  qualitative = NA,                 ### Description (qualitative research)
  comment = "scale ranges from 20 to 100"                      ### Comment or NA
...
res$variable[[length(res$variable) + 1]] <- list(
  variable = "temptation",             ### Replace 'example' with variable name
  moment = 1,                       ### moment this variable was measured/manipulated
  type = "question",                ### "question", "aggregate", or "manipulation"
  datatype = "numeric",             ### "numeric", "logical", "nominal", "ordinal", or "text"
  values = NA,                      ### Possible values; NA or a vector, e.g. c(1, 2, 3)
  labels = NA,                      ### Labels for the values; NA or e.g. c("no", "maybe", "sometimes")
  description = "Alcohol Abstinence Self-Efficacy (AASE) Scale to evaluate an individual's perceived temptation to drink. Scores range from 20 to 100",					###	Provide the full description of the variable (e.g., questionnaire question)
...
####################################### ONLY FOR DETERMINANTS OF MATERNAL DRINKING BEHAVIOR VARIABLES
  ...
  determinant_recode = "barriers, temptations",		### Determinant recoded after expert meeting 
...
res$variable[[length(res$variable) + 1]] <- list(
  variable = "confidence",             ### Replace 'example' with variable name
  moment = 1,                       ### moment this variable was measured/manipulated
  type = "question",                ### "question", "aggregate", or "manipulation"
  datatype = "numeric",             ### "numeric", "logical", "nominal", "ordinal", or "text"
  values = NA,                      ### Possible values; NA or a vector, e.g. c(1, 2, 3)
  labels = NA,                      ### Labels for the values; NA or e.g. c("no", "maybe", "sometimes")
  description = "Alcohol Abstinence Self-Efficacy (AASE) Scale to evaluate an individual's perceived confidence to abstain from alcohol. Scores range from 20 to 100",					###	Provide the full description of the variable (e.g., questionnaire question)
