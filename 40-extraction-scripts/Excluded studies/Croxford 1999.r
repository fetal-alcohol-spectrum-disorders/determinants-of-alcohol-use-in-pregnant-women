...
res$sample$samplingDesign <- "prospective";	 ### 'longitudinal' vs 'cross-sectional'
...
res$sample$recruitmentSetting <- "antenatal clinics"; ### For example "antenatal clinic",or NA

### Sample Method
res$sample$native <- NA; ### TRUE for native/aboriginal populations (e.g. inuit, aboriginals, etc)

### Description of sample, in text
res$sample$description <- "pregnant women both drinkers and non drinkers"

### Description of geography of sample, in text
res$sample$geography <- "South Africa"

### Study year, in text
res$sample$year <- 1995;
res$sample$endyear <- 1996;
...
res$univariate[[length(res$univariate) + 1]] <- list(
  variable = "aware of harmful effects alcohol for fetus",       ### Name of variable this result pertains to
  ...
  mean = NA,                        ### Mean or NA
  percentage = 59.7,                  ### Percentage that endorsed 1, or NA
...
res$variable[[length(res$variable) + 1]] <- list(
  variable = "aware of harmful effects alcohol for fetus",             ### Replace 'example' with variable name
  moment = 1,                       ### moment this variable was measured/manipulated
  type = "question",                ### "question", "aggregate", or "manipulation"
  datatype = "numeric",             ### "numeric", "logical", "nominal", "ordinal", or "text"
  values = NA,                      ### Possible values; NA or a vector, e.g. c(1, 2, 3)
  labels = NA,                      ### Labels for the values; NA or e.g. c("no", "maybe", "sometimes")
  description = "the survey asked specifically whether mothers were aware of any factors that were harmful or could cause problems for their fetuses in utero(p.964)",					###	Provide the full description of the variable (e.g., questionnaire question)
