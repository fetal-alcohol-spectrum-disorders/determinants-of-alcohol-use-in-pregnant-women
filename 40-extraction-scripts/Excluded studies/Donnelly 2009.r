...
res$sample$description <- "postpartum women"

### Description of geography of sample, in text
res$sample$geography <- "For example 'Netherlands'"

### Study year, in text
res$sample$year <- 1993;
...
res$univariate[[length(res$univariate) + 1]] <- list(
  variable = "no known amount alcohol safe to drink",       ### Name of variable this result pertains to
  ...
  mean = NA,                        ### Mean or NA
  percentage = 77/84*100,                  ### Percentage that endorsed 1, or NA
  sd = NA,                          ### Standard deviation, or NA
  qualitative = NA,                 ### Description (qualitative research)
  comment = "92% of the women (77/84) knew that no safe level of drinking in pregnancy has been established, p.680"                      ### Comment or NA
);
##########################################################################
...
res$variable[[length(res$variable) + 1]] <- list(
  variable = "no known amount alcohol safe to drink",             ### Replace 'example' with variable name
  moment = 1,                       ### moment this variable was measured/manipulated
  type = "question",                ### "question", "aggregate", or "manipulation"
  datatype = "numeric",             ### "numeric", "logical", "nominal", "ordinal", or "text"
  values = NA,                      ### Possible values; NA or a vector, e.g. c(1, 2, 3)
  labels = NA,                      ### Labels for the values; NA or e.g. c("no", "maybe", "sometimes")
  description = "How much alcohol can a pregnant woman safely consume (answer: none) (p.677 table 1)",	###	Provide the full description of the variable (e.g., questionnaire question)
